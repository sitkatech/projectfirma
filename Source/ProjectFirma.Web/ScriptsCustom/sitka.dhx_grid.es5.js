/*-----------------------------------------------------------------------
<copyright file="sitka.grid.js" company="Tahoe Regional Planning Agency and Environmental Science Associates">
...
////    var self = this;
////    var dhxGrid = this.SitkaGridContainer.grid;
...
////    var myCalendar = new dhXCalendarObject([datefrom, dateto]);
////    myCalendar.setDateFormat("%m/%d/%Y");      //Date format MM/DD/YYY
////    myCalendar.attachEvent("onClick", function (date) { dhxGrid.filterByAll(); });
...
////this is pretty much the same thing as dhxGridObjects Numeric_filter but with a stripNonNumeric function when evaluating cell content.
...
Sitka.DHX_Grid.Class = {
    GridColumn: function GridColumn(columnName, title, width, align, type, sorting, filtertype, formatType) {
        this.width = width;
        this.id = columnName;
        this.header = [{ text: columnName }];
    ...
    Grid: function Grid(gridName, gridElement, dataElement, scrollPosition, skin, skinRowHeight, saveGridSettingsUrl) {
        if (arguments.length != 7) {
            throw new Error("Expects exactly 7 arguments Sitka.DHX_Grid.Grid constructor but got " + arguments.length);
        }
        ...
        if (gridElement === undefined || gridElement === null) {
            throw new Error("Required parameter gridElement not provided Sitka.DHX_Grid.Grid constructor.");
        }
        if (dataElement === undefined) {
            // OK if this one is null
            throw new Error("Required parameter dataElement not provided Sitka.DHX_Grid.Grid constructor.");
        }
        if (scrollPosition === undefined || scrollPosition === null) {
            throw new Error("Required parameter scrollPosition not provided Sitka.DHX_Grid.Grid constructor.");
        }
        if (skin === undefined || skin === null) {
            throw new Error("Required parameter skin not provided Sitka.DHX_Grid.Grid constructor.");
        }
        if (skinRowHeight === undefined || skinRowHeight === null) {
            throw new Error("Required parameter skinRowHeight not provided Sitka.DHX_Grid.Grid constructor.");
        }
        ...
        this._columns = [];
        this.grid = null;
        this.grid = new dhx.Grid(this.gridElement, this._columns);

        //this.grid.setImagePath("/content/dhx/dhxGrid/codebase/imgs/");
...
Sitka.DHX_Grid.Class.Grid.prototype.setColumns = function (gridColumns) {
    this._columns = _(gridColumns);
    myList = [];
    gridColumns.forEach(function (e) {
        myList.push(_.mapValues(e));
    });
    this.grid.setColumns(myList);
};

//Sitka.DHX_Grid.Class.Grid.prototype.getColumnIndexByName = function (columnName) {
//    return this._columns.pluck("columnName").indexOf(columnName);
//};

////Sitka.DHX_Grid.Class.Grid.prototype.htmlFilter = function (t, i) {
...
////    };
////};
////Sitka.DHX_Grid.Class.Grid.prototype.formattedNumericFilter = function (t, i) { var g = t; };

////TODO: Develop tests and handle error cases
//Sitka.DHX_Grid.Class.Grid.prototype.getValuesFromCheckedGridRows = function (checkBoxColumnIndex, valueColumnName, returnListName) {
...
//// we want to intercept the call when dhx builds up the list of select option items, to transform it into a multiselect
...
//Sitka.DHX_Grid.Class.Grid.prototype.strictHtmlFilter = function (t, i) {
...
//    var tObj = jQuery(t);
//    var jSelect = tObj.find("select.dhx-grid-bootstrap-multiselect-filter");
//    var jSelectElement = tObj.find("select.dhx-grid-bootstrap-multiselect-filter").get()[0];
...
//// This is an edited version of _in_header_select_filter_strict found in dhxgrid_filter.js, it adds multiselect attributes to the select element and creates a multiselect object, importantly it omits click handler for parent cell onclick
//var sitkaGridLocallyDefinedMultiSelectFilter = function (t, i) {
//    t.innerHTML = "<select style='width:90%; font-size:8pt; font-family:Tahoma;' class='dhx-grid-bootstrap-multiselect-filter' multiple='multiple'></select>";
//    var tObj = jQuery(t);
//    tObj.addClass("dhx-grid-bootstrap-multiselect-filter-container");
//    var jSelect = tObj.find(".dhx-grid-bootstrap-multiselect-filter");
...
//// we want to redefine everything from dhx that renders a select to use our multi-select for a consistent appearance:
...
//Sitka.DHX_Grid.Class.Grid.prototype.textWithDashesFilter = function (t, i) {
//    dhx.Grid.prototype._in_header_text_filter.call(this, t, i);
//    t.firstChild._filter = function () {
//        return function (value) {
//            var filterData = t.firstChild.value;
...
//Sitka.DHX_Grid.Class.Grid.prototype.sortOnOtherColumn = function (originalColumn, sortColumn) {
...
//Sitka.DHX_Grid.Class.Grid.prototype.filterOnOtherColumn = function (originalColumn, filterColumn) {
...
//Sitka.DHX_Grid.Class.Grid.prototype.showHideFilterRow = function (setToState) {
...
//Sitka.DHX_Grid.Class.Grid.prototype.showHideFilterInstructions = function () {
...
////Sitka.DHX_Grid.Class.Grid.prototype.loadFilterRowState = function () {
...
//Sitka.DHX_Grid.Class.Grid.prototype.setColumnNumberFormats = function (columnFormatTypes, formatTypeToMatch, numberFormat) {
...
Sitka.DHX_Grid.Class.Grid.prototype.buildWithArguments = function (hideHeader, groupingHeader, filterHeader, dataUrl, useSmartRendering, splitAtColumn, initWidthByPercentage) {
    ...
    //    jQuery("#" + theGridElement).find(".dhx-grid-bootstrap-multiselect-filter-container").find(".btn-group.open .dropdown-toggle").dropdown("toggle");
    //};
    ...
    //// dhx wants to take over all the click events so you have to sneak in an event handler where its not looking
    ...
    //// encompass clicks on the actual table data. https://docs.dhx.com/api__dhxgrid_onemptyclick_event.html
    ...
    //// Again, dhx wants to take over so many click events. Because the above onEmptyClick doesn't do anything when clicking on the table data
    ...
    //        //  For dhxgrid smart rendering height has to be the same between row content (image heights), sitka_dhxgrid_skins.css (.ev_sitkaskin, .odd_sitkaskin), and sitka.js (grid.setAwaitedRowHeight) or paged grids scrolling will be off in dhxgrid smart rendering mode
...
//Sitka.DHX_Grid.Class.Grid.prototype.buildWithDataUrl = function (hideHeader, groupingHeader, dataUrl) { this.buildWithArguments(hideHeader, groupingHeader, null, dataUrl); };

//Sitka.DHX_Grid.Class.Grid.prototype.load = function (dataUrl) {
...
//Sitka.DHX_Grid.Class.Grid.prototype.setGridInstructions = function (instructionHtml, show) {
...
//Sitka.DHX_Grid.Class.Grid.prototype.showGridInstructions = function () {
//    var objbox = jQuery(this.grid.objBox);
//    jQuery(".gridInstruction", objbox).show();
//    objbox.addClass("gridInstructionPane");
//};

//Sitka.DHX_Grid.Class.Grid.prototype.hideGridInstructions = function () {
//    var objbox = jQuery(this.grid.objBox);
//    jQuery(".gridInstruction", objbox).hide();
//    objbox.removeClass("gridInstructionPane");
//};

//Sitka.DHX_Grid.Class.Grid.prototype.build = function (hideHeader, groupingHeader, filterHeader) { this.buildWithArguments(hideHeader, groupingHeader, filterHeader, null); };

//Sitka.DHX_Grid.Class.Grid.prototype.formatEdit = function (rowId, columnIndex) { this.grid.rowsAr[rowId].cells[columnIndex].className += " sitkaGridEditable"; };

//Sitka.DHX_Grid.Class.Grid.prototype.getFilterValuesFromGrid = function () {
//    var filterValues = [];
...
//Sitka.DHX_Grid.Class.Grid.prototype.hasSavedFilters = function () {
...
Sitka.DHX_Grid.Class.Grid.prototype.setFilteringButtonTagName = function (filterTagName) {/*this.filterTagName = "#" + filterTagName;*/};

//Sitka.DHX_Grid.Class.Grid.prototype.showOrHideFilteringButton = function () {
...
//Sitka.DHX_Grid.Class.Grid.prototype.resetMultiSelectFilters = function () {
...
//Sitka.DHX_Grid.Class.Grid.prototype.clearGridFilters = function () {
...
//Sitka.DHX_Grid.Class.Grid.prototype.setGridFromSavedValues = function (filterValuesArray) {
//    this.grid.refreshFilters();
...
Sitka.DHX_Grid.Class.Grid.prototype.setupFilterCountElement = function (filterCountElementId) {
    //var self = this;
...
//Sitka.DHX_Grid.Class.Grid.prototype.setupFilterDownloadElement = function (filterDownloadElementId) {
//    var self = this;
...
//Sitka.DHX_Grid.Class.Grid.prototype.updateFilterCountElement = function () {
//    if (this.filterCountElement) {
//        this.filterCountElement.html(this.grid.getRowsNum());
//    }
//};

//Sitka.DHX_Grid.Class.Grid.prototype.updateSelectedCheckboxCount = _.debounce(function () {
...
//Sitka.DHX_Grid.Class.Grid.prototype.updateFilterDownloadElement = function () {
...
//Sitka.DHX_Grid.Class.Grid.prototype.retrieveValuesFromColumn = function (columnName) {
...
//Sitka.DHX_Grid.Class.Grid.prototype.resizeGridWithVerticalFill = function ()
//{
...
//Sitka.DHX_Grid.Class.Grid.prototype.resizeGridWidths = function ()
//{
//    var gridDiv = jQuery("#" + this.gridElement);
...
Sitka.DHX_Grid.Class.Grid.prototype.setupServerFilterSaving = function (filterElementId) {
    //var self = this;
...
//Sitka.DHX_Grid.Class.Grid.prototype.getColumnSortTypeByIndex = function (columnIndex) {
//    return this._columns.pluck("sorting").value()[columnIndex];
//};

//Sitka.DHX_Grid.Class.Grid.prototype.asGridTableForSettings = function () {
...
//    return tableToPost;
//}
//Sitka.DHX_Grid.Class.Grid.prototype.sitkaGridToExcel = function (url) {
//    var self = this;

//    this.saveFiltersToServer(this.grid.toExcel(url));

//};
//Sitka.DHX_Grid.Class.Grid.prototype.saveFiltersToServer = function (callbackFunction) {
