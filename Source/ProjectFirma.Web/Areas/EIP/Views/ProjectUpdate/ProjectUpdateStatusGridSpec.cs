...
namespace ProjectFirma.Web.Areas.EIP.Views.ProjectUpdate
{
    public class ProjectUpdateStatusGridSpec : GridSpec<Models.Project>
    {
            ...
            bool canApprove)
        {
            ...
            Add(FieldDefinition.ProjectNumber.ToGridHeaderString(), x => UrlTemplate.MakeHrefString(x.GetSummaryUrl(), x.ProjectNumberString), 80, DhtmlxGridColumnFilterType.Html);
            Add(FieldDefinition.ProjectName.ToGridHeaderString(), x => UrlTemplate.MakeHrefString(x.GetSummaryUrl(), x.ProjectName), 180, DhtmlxGridColumnFilterType.Html);
            Add(FieldDefinition.LeadImplementer.ToGridHeaderString(), x => UrlTemplate.MakeHrefString(x.LeadImplementer != null ? x.LeadImplementer.GetSummaryUrl() : null, x.LeadImplementerName), 130);
            Add(FieldDefinition.PrimaryContact.ToGridHeaderString(),
                x => x.PrimaryContactPerson == null ? new HtmlString(string.Format("({0})", ViewUtilities.NoneString)) : x.PrimaryContactPerson.GetFullNameFirstLastAndOrgAsUrl(),
                95);
            Add(FieldDefinition.Stage.ToGridHeaderStringWider(), x => x.ProjectStage.ProjectStageDisplayName, 80, DhtmlxGridColumnFilterType.SelectFilterStrict);
            ...
            Add(FieldDefinition.SecuredFunding.ToGridHeaderString(), x => x.SecuredFunding, 95, DhtmlxGridColumnFormatType.Currency, DhtmlxGridColumnAggregationType.Total);
