using System.Linq;
using System.Web;
using LtInfo.Common;
using LtInfo.Common.DhtmlWrappers;
using LtInfo.Common.HtmlHelperExtensions;
using LtInfo.Common.Views;

namespace ProjectFirma.Web.Areas.EIP.Views.FocusArea
{
    public class IndexGridSpec : GridSpec<Models.FocusArea>
    {
        public IndexGridSpec(bool hasDeletePermissions)
        {            
            if (hasDeletePermissions)
            {
                Add(string.Empty, x => DhtmlxGridHtmlHelpers.MakeDeleteIconAndLinkBootstrap(x.DeleteUrl, true, !x.HasDependentObjects()), 30);
            }

            Add("#", a => UrlTemplate.MakeHrefString(a.SummaryUrl, a.DisplayNumber), 60, DhtmlxGridColumnFilterType.Html);
            Add(Models.FieldDefinition.FocusArea.ToGridHeaderString(), a => UrlTemplate.MakeHrefString(a.SummaryUrl, a.FocusAreaName), 240);
            Add(Models.FieldDefinition.Program.ToGridHeaderString(), a => new HtmlString(string.Join("<br/>", a.Programs.Select(x => x.DisplayNameAsUrl))), 340, DhtmlxGridColumnFilterType.Html);
            Add("# of Projects", a => a.Projects.Count, 90);
        }
    }
}