...
{
    [DataContract]
    public class WebServiceEIPPerformanceMeasure : SimpleModelObject
    {
        public WebServiceEIPPerformanceMeasure(EIPPerformanceMeasure eippm)
        {
            var indicator = eippm.Indicator;
            ...
            if (eippm.Indicator.HasRealSubcategories)
            {
                var currentSubcategoryIndex = 1;
                foreach (var indicatorSubcategory in eippm.IndicatorSubcategories)
                {
        ...
        public static List<WebServiceEIPPerformanceMeasure> GetEIPIndicators()
        {
            var indicators = HttpRequestStorage.DatabaseEntities.EIPPerformanceMeasures.ToList();
            return indicators.Select(x => new WebServiceEIPPerformanceMeasure(x)).OrderBy(x => x.IndicatorID).ToList();
        }
    }

    public class WebServiceEIPPerformanceMeasureGridSpec : GridSpec<WebServiceEIPPerformanceMeasure>
    {
        public WebServiceEIPPerformanceMeasureGridSpec()
        {
