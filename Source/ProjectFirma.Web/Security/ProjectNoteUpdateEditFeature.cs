/*-----------------------------------------------------------------------
<copyright file="ProjectNoteUpdateEditFeature.cs" company="Tahoe Regional Planning Agency and Environmental Science Associates">
...
{
    [SecurityFeatureDescription("Edit Notes for {0} Updates", FieldDefinitionEnum.Project)]
    public class ProjectNoteUpdateEditFeature : FirmaFeatureWithContext, IFirmaBaseFeatureWithContext<ProjectNoteUpdate>
    {
        ...
        public void DemandPermission(FirmaSession firmaSession, ProjectNoteUpdate contextModelObject)
        {
            _firmaFeatureWithContextImpl.DemandPermission(firmaSession, contextModelObject);
        }

        public PermissionCheckResult HasPermission(FirmaSession firmaSession, ProjectNoteUpdate contextModelObject)
        {
            var hasPermissionByFirmaSession = HasPermissionByFirmaSession(firmaSession);
            if (!hasPermissionByFirmaSession)
            {
                return new PermissionCheckResult($"You don't have permission to Edit {FieldDefinitionEnum.Project.ToType().GetFieldDefinitionLabel()} {contextModelObject.ProjectUpdateBatch.Project.GetDisplayName()}");
            }
