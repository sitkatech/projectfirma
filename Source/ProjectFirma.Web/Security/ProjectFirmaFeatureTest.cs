...
namespace ProjectFirma.Web.Security
{
    ...
    public class ProjectFirmaFeatureTest
    {
        ...
        public void AllContextFeaturesImplementInterface()
        {
            ...
            var iProjectFirmaFeatureTypeWithContextTypes = AppDomain.CurrentDomain.GetAssemblies().SelectMany(s => s.GetTypes()).Where(p => p.GetInterfaces().Any(i => HasInterface(i, _typeofIFirmaBaseFeatureWithContext))).Select(t => t.FullName).ToList();

            //The two lists should be the same
            Assert.That(projectFirmaFeatureWithContextTypes, Is.EquivalentTo(iProjectFirmaFeatureTypeWithContextTypes), string.Format("All features of type {0} must implement {1}", _typeOfEIPFeatureWithContext.Name, _typeofIFirmaBaseFeatureWithContext.Name));
        }
