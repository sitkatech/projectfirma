/*-----------------------------------------------------------------------
<copyright file="ProjectClassificationController.cs" company="Tahoe Regional Planning Agency and Environmental Science Associates">
    ...
    public class ProjectClassificationController : FirmaBaseController
    {
            ...
            ClassificationSystem classificationSystem)
        {
            ...
            var projectClassificationSimples =
                HttpRequestStorage.DatabaseEntities.Classifications.ToList().Where(x => x.ClassificationSystem == classificationSystem).OrderBy(x => x.GetSortOrder()).ThenBy(x => x.GetDisplayName()).Select(x => new ProjectClassificationSimple {ClassificationID = x.ClassificationID}).ToList();
