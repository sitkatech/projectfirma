            ...
            {
                {
                    ...
                    ProjectMapCustomization.GetProjectStagesForMap(showProposals)
                        .Where(x => x.ShouldShowOnMap()).OrderBy(x => x.SortOrder).Select(x => new ProjectMapLegendElement(x.ProjectStageID, x.GetProjectStageColor(), x.GetProjectStageDisplayName())).ToList()
                }
            };
