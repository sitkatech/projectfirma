...
namespace ProjectFirma.Web.Views.Shared.MatchmakerOrganizationControls
{
    public class MatchmakerKeywordHelper
    {
        ...
        {
            Keywords = keywords;
            AddKeywordsUrl = SitkaRoute<TagController>.BuildUrlFromExpression(c => c.AddTagsToProject());
            RemoveKeywordsUrl = SitkaRoute<TagController>.BuildUrlFromExpression(c => c.RemoveTagsFromProject());
            FindKeywordsUrl = SitkaRoute<TagController>.BuildUrlFromExpression(c => c.Find(null));
        }
    }
}
