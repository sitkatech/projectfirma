/*-----------------------------------------------------------------------
<copyright file="EditSortOrderInGroupViewModel.cs" company="Tahoe Regional Planning Agency and Environmental Science Associates">
...
{
    public class EditSortOrderInMenuGroupViewModel : FormViewModel
    {
        public const int SortOrderIncrement = 10;

        public List<int> ReorderedSortableIDs { get; set; }

        public EditSortOrderInMenuGroupViewModel()
        {
        }
        ...
        public void UpdateModel(List<IHaveASortOrder> customPages)
        {
            
                for (var i = 0; i < ReorderedSortableIDs.Count; i++)
                {
                    var current = customPages.SingleOrDefault(x => x.GetID() == ReorderedSortableIDs[i]);
