...
{
    /// <summary>
    /// All the logic to get the ProjectFirma Scheduled Jobs wired up in <see cref="Hangfire"/>
    /// </summary>
    public class ScheduledBackgroundJobBootstrapper
    {
        /// <summary>
        /// Configuration entry point for <see cref="ProjectFirmaOwinStartup"/> via the <see cref="OwinStartupAttribute"/>
        /// </summary>
        ...
        private static void ConfigureHangfire(IAppBuilder app)
        {
            ...
            GlobalConfiguration.Configuration.UseSqlServerStorage(ProjectFirmaWebConfiguration.DatabaseConnectionString, sqlServerStorageOptions);
            app.UseHangfireServer(new BackgroundJobServerOptions { WorkerCount = 1 }); // 11/03/2015 MF - limit the number of worker threads, we really don't need that many - in fact we try to have each job run serially for the time being because we're not sure how concurrent the different jobs could really be.
            ...
            {
                AuthorizationFilters = new[] { new HangfireProjectFirmaWebAuthorizationFilter() }
            });
        }
        ...
        private static void ConfigureScheduledBackgroundJobs()
        {
            ...
            // 1:00 AM tasks
            AddRecurringJob(ProjectUpdateKickoffReminderScheduledBackgroundJob.Instance.JobName,
                () => ScheduledBackgroundJobLaunchHelper.RunProjectUpdateKickoffReminderScheduledBackgroundJob(),
                MakeYearlyUtcCronJobStringFromLocalTime(11, 1, 1, 0),
                recurringJobIds);
            AddRecurringJob(ProjectUpdateSecondReminderScheduledBackgroundJob.Instance.JobName,
                () => ScheduledBackgroundJobLaunchHelper.RunProjectUpdateSecondReminderScheduledBackgroundJob(),
                MakeYearlyUtcCronJobStringFromLocalTime(12, 1, 1, 0),
                recurringJobIds);
            AddRecurringJob(ProjectUpdateThirdReminderScheduledBackgroundJob.Instance.JobName,
                () => ScheduledBackgroundJobLaunchHelper.RunProjectUpdateThirdReminderScheduledBackgroundJob(),
                MakeYearlyUtcCronJobStringFromLocalTime(1, 5, 1, 0),
                recurringJobIds);
            AddRecurringJob(ProjectUpdateNearingDeadlineReminderScheduledBackgroundJob.Instance.JobName,
                () => ScheduledBackgroundJobLaunchHelper.RunProjectUpdateNearingDeadlineReminderScheduledBackgroundJob(),
                MakeYearlyUtcCronJobStringFromLocalTime(1, 10, 1, 0),
                recurringJobIds);
            AddRecurringJob(ProjectUpdateAtDeadlineReminderScheduledBackgroundJob.Instance.JobName,
                () => ScheduledBackgroundJobLaunchHelper.RunProjectUpdateAtDeadlineReminderScheduledBackgroundJob(),
                MakeYearlyUtcCronJobStringFromLocalTime(1, 15, 1, 0),
                recurringJobIds);
            AddRecurringJob(ProjectUpdatePastDeadlineReminderScheduledBackgroundJob.Instance.JobName,
                () => ScheduledBackgroundJobLaunchHelper.RunProjectUpdatePastDeadlineReminderScheduledBackgroundJob(),
                MakeDailyUtcCronJobStringFromLocalTime(1, 0),
                recurringJobIds);
            // Remove any jobs we haven't explicity scheduled
            RemoveExtraneousJobs(recurringJobIds);
        }

        private static void AddRecurringJob(string jobName, Expression<Action> methodCallExpression, string cronExpression, List<string> recurringJobIds)
        {
