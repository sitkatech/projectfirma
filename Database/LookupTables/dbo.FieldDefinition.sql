...
(314, N'ExternalMapLayerDisplayName', 'Display Name', 'The layer display name will appear in map legends and popups.'),
(315, N'ExternalMapLayerUrl', 'Url', 'The external map web service URL (E.g https://hydro.nationalmap.gov/arcgis/rest/services/wbd/MapServer/3 (National 6-digit HUC boundaries).'),
(316, N'ExternalMapLayerDescription', 'Internal Layer Description', 'Add helpful background information for other administrators (E.g who to contact if there is a problem with this external map service in the future).'),
(317, N'ExternalMapLayerFeatureNameField', 'Field to use as source for feature names', 'This setting will populate the map pop-ups with names from the feature service. This is field is case-sensitive (E.g. Name or NAME) and must match a field in the web service. Please leave this field blank for tile services since pop-ups are not supported.'),
(318, N'ExternalMapLayerDisplayOnAllMaps', 'Display on all maps?', 'When this option is set, display the external map layer on various maps throughout the site similar to geospatial areas (e.g watersheds). When not set, the external map layer will only appear on the Project Map.'),
(319, N'ExternalMapLayerLayerIsOnByDefault', 'Layer is on by default?', 'This setting determines whether the layer is visible by default on the maps on which it appears.'),
(320, N'ExternalMapLayerIsActive', 'Is Active?', 'If this is not set, the layer will not appear on any maps. If there is a problem with the external map service an administrator can uncheck this box to hide the layer until it is resolved.'),
(321, N'ExternalMapLayerIsATiledMapService', 'Is a Tiled Map Service?', 'Please turn on this setting if the external map layer is a tiled layer (raster). Note: Pop-ups will not appear on maps for tiled layers.'),
(322, N'Evaluation', 'Evaluation', N'')